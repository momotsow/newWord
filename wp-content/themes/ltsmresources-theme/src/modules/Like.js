...
class Like {
  ...
  async createLike(currentLikeBox) {
    try {
      const response = await axios.post(ltsmData.root_url + "/wp-json/ltsm/v1/manageLike", { "professorId": currentLikeBox.getAttribute("data-professor") })
  ...
  async deleteLike(currentLikeBox) {
    try {
      const response = await axios({
        url: ltsmData.root_url + "/wp-json/ltsm/v1/manageLike",
        method: 'delete',
